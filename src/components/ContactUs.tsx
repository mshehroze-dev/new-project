...
import { supabase } from '@/lib/supabase';import { useAuth } from '@/components/auth/AuthProvider';
...
const ContactUs: React.FC = () => {  const { user } = useAuth();  const contactRecipient = (import.meta.env.VITE_CONTACT_EMAIL as string | undefined) || ''
  ...
  const canSend = useMemo(() => {    return Boolean(user && contactRecipient)  }, [user, contactRecipient])
          ...
          >
            <div className="space-y-4">
              ...
              <p className="mt-4 text-sm text-slate-500">
                Please <Link className="font-semibold text-indigo-600" to="/login">sign in</Link> to send a message.
              </p>
